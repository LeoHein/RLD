...
class NN(nn.Module):
    def __init__(self, inSize, outSize, layers=[], finalActivation=None, dueling=False, activation=torch.tanh,dropout=0.0):
...
class NNactor(nn.Module):
    def __init__(self, inSize, outSize, layers=[], finalActivation=None, dueling=False, activation=torch.tanh,dropout=0.0):
        ...
        for x in layers:
            self.norms.append(nn.BatchNorm1d(inSize))
            self.layers.append(nn.Linear(inSize, x))
            inSize = x
        self.norms.append(nn.BatchNorm1d(inSize))
        self.mu_layer = nn.Linear(inSize, outSize)
        self.log_sigma_layer = nn.Linear(inSize, outSize)
        self.activation = activation
        self.finalActivation = finalActivation
        self.dropout = None
        if dropout > 0:
    ...
    def forward(self, x):
        ...
        if self.finalActivation is not None:
            mu=self.finalActivation(mu)
            log_sigma=self.finalActivation(log_sigma)
...
class NNcriticImp(nn.Module):
    def __init__(self, inSize, outSize, layers=[], finalActivation=None, dueling=False, activation=torch.tanh,
                 dropout=0.0):
        super(NNcriticImp, self).__init__()
        self.layers = nn.ModuleList([])
        self.norms = nn.ModuleList([])
        for x in layers:
            self.norms.append(nn.BatchNorm1d(x))
            self.layers.append(nn.Linear(inSize, x))
            inSize = x
        self.norms.append(nn.BatchNorm1d(outSize))
        self.layers.append(nn.Linear(inSize, outSize))
        self.activation = activation
        self.finalActivation = finalActivation
        self.dropout = None
    ...
    def forward(self, x):
        ...
        for i in range(1, len(self.layers)):
            ...
            if self.dropout is not None:
                x = self.dropout(x)
            # print("x",x.shape)
            # print("x then ", x.shape)
            x = self.layers[i](x)
            x = self.norms[i](x)
            # print("x then then", x.shape)
